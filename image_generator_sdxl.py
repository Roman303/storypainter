...
def download_civitai_model(version_id: str, model_dir: Path) -> Path:
    """Lädt Model von CivitAI herunter (mit API-Key)"""
    
    # ──────────────────────────────────────────────────
    # 🔑 API-Key festlegen
    # Ersetze "dein-echter-api-key" mit deinem echten Key
    # oder nutze: CIVITAI_API_KEY = os.environ.get("CIVITAI_API_KEY", "fallback-key")
    # ──────────────────────────────────────────────────
    CIVITAI_API_KEY = "dein-echter-api-key"
    
    model_dir.mkdir(parents=True, exist_ok=True)
    print(f"📡 Hole Model-Info von CivitAI (Version ID: {version_id})...")
    
    # Basis-Headers für alle Requests (Auth, wenn Key vorhanden)
    base_headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
    }
    if CIVITAI_API_KEY:
        base_headers['Authorization'] = f'Bearer {CIVITAI_API_KEY}'
    
    try:
        # 1. API-Aufruf: Modell-Informationen holen
        api_url = f"https://civitai.com/api/v1/model-versions/{version_id}"
        # Für JSON-Antwort explizit Accept setzen
        ...
        # Sicherstellen, dass keine HTML-Seite zurückkommt
        content_type = response.headers.get('Content-Type', '')
        if 'text/html' in content_type:
            raise Exception("CivitAI lieferte HTML statt JSON (wahrscheinlich Auth-Problem)")
        ...
        # Download-Headers (Accept für Binärdaten anpassen)
        dl_headers = base_headers.copy()
        dl_headers['Accept'] = '*/*'
        dl_headers['Referer'] = f'https://civitai.com/models/{data.get("model", {}).get("id", "")}'
        
        # 3. Download durchführen
        response = requests.get(download_url, headers=dl_headers, timeout=30, stream=True)
        
        # Falls dennoch 401 kommt (veralteter Key o.ä.), alternative Download-URL versuchen
        if response.status_code == 401:
            ...
            for file_info in files:
                if file_info.get('primary', False):
                    ...
                    if direct_url:
                        print("ℹ️  Verwende alternative Download-URL...")
                        response = requests.get(direct_url, headers=dl_headers, timeout=30, stream=True)
                        break
        ...
        print(f"ℹ️  Bitte überprüfe deinen API-Key oder lade das Model manuell herunter.")
        # Bekannte Fallback-Datei prüfen (hartcodiert, optional)
        manual_path = model_dir / "NeoNoirCinema_SinCityStyle_SDXL.safetensors"
        if manual_path.exists():
            print(f"✅ Lokale Datei gefunden: {manual_path}")
            return manual_path
        raise
