...
class SceneBasedAudiobookGenerator:
    ...
    def ensure_whisper_loaded(self):
        ...
        print(f"\n🔥 Lade Whisper QC-Modell ({whisper_model}, device={device}, compute_type={compute_type})...")
        try:
    ...
    def generate_chunk_with_qc(self, tts, chunk_text, scene_id, chunk_id, part_idx=None):
        ...
        if self.whisper is None:
            ...
            return False, {"cer": None, "attempts": 1, "transcript": None}
    ...
    def generate_audiobook_from_scenes(self):
        ...
        for scene_idx, scene in enumerate(scenes, 1):
            ...
            for chunk_idx, chunk_text in enumerate(chunks, 1):
                ...
                # === LOW MODE 2: Sub-Chunking NACH Existenz-Check ===
                if low_mode == 2:
                    # Teile Chunk in 2 Hälften
                    mid = len(chunk_text) // 2
                    # Suche nächsten Satzende/Komma in der Nähe der Mitte
                    split_pos = mid
                    for i in range(mid, min(mid + 80, len(chunk_text))):
                        if chunk_text[i] in '.!?,;':
                            split_pos = i + 1
                            break
                    
                    sub_chunks = [
                        chunk_text[:split_pos].strip(),
                        chunk_text[split_pos:].strip()
                    ]
                    sub_chunks = [s for s in sub_chunks if s]
                    
                    print(f"   [{chunk_idx:03d}] ✂️ LOW MODE 2: Teile in {len(sub_chunks)} Sub-Chunks")
                    ...
                    for sub_i, sub_text in enumerate(sub_chunks, 1):
                        ...
                        start = time.time()
                        success, qc_info = self.generate_chunk_with_qc(
                            tts, sub_text, scene_id, chunk_idx, part_idx=sub_i
                        )
                        duration = time.time() - start
                        
                        if success:
                            cer_str = f"{qc_info['cer']:.3f}" if qc_info.get('cer') else "n/a"
                    ...
                    if success:
                        ...
                        print(f"           ⚠️ QC nicht bestanden (CER={qc_info.get('cer', 'n/a')})")
...
def main():
    ...
    CONFIG = {
        ...
        # QC deaktiviert (kein Whisper)
        "whisper_model_name": None,
        "qc_cer_threshold": 999,
        "max_silence_sec": 1.0,
    ...
    if low_mode == 1:
        ...
        print("\n🔧 LOW MODE 2: Extra-Stabil (halbe Chunks) aktiviert")
        CONFIG.update({
            "temperature": 0.50,
            "top_p": 0.88,
            "top_k": 30,
            "repetition_penalty": 1.55,
            "max_chunk_length": 120,  # Halbiert!
        })
