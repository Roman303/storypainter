...
class UltraQualitySDXL:
    ...
    # IMAGE GENERATION
    #############################################
    def generate(self, prompt, seed=42, save_intermediates=False, intermediate_dir="intermediates", every_n_steps=5):
        ...
        #############################################
        # BASE PHASE (with optional intermediates)
        #############################################
        # activate intermediate saving
        if save_intermediates:
            os.makedirs(intermediate_dir, exist_ok=True)

            def save_step_callback(pipe, i, t, latents):
                if i % every_n_steps == 0:
                    img = pipe.decode_latents(latents)[0]
                    img.save(os.path.join(intermediate_dir, f"base_step_{i:03d}.png"))

            self.base._callbacks = [save_step_callback]
            self.base._callback_steps = 1
