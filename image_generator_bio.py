#!/usr/bin/env python3
"""
SDXL V14 - MEMORY-OPTIMIERT für 16GB GPU
Fotorealistische Pflanzenbilder mit Detail- und Stil-LoRAs
...
class LoRAManager:
    ...
    DEFAULT_LORA_PATHS = {
        "detail_tweaker": {
            ...
            "description": "Verbessert feine Details wie Blattadern und Texturen"
        },
        "realistic_vision": {
            "path": "./loras/sdxl/realistic_vision_xl.safetensors",
            "scale": 0.65,
            "trigger": "photorealistic, professional photography",
            "description": "Erhöht fotografische Qualität"
        },
        "cinematic_lighting": {
            "path": "./loras/sdxl/cinematic_lighting_xl.safetensors",
            "scale": 0.55,
            "trigger": "cinematic lighting, dramatic lighting",
            "description": "Professionelle Beleuchtung"
        },
        "botanical_focus": {
            "path": "./loras/sdxl/botanical_focus_xl.safetensors",
            "scale": 0.7,
            "trigger": "botanical details, plant texture",
            "description": "Spezialisiert auf Pflanzen"
        }
    }
...
class BotanicalSDXLLoRA:
    def __init__(
        ...
        print("🌿 Initialisiere BOTANICAL SDXL (16GB optimiert)...")
        ...
        # BOTANICAL PROMPT TEMPLATES
        self.botanical_templates = {
            "macro_plant": {
                "base": "macro photography of {plant}, extreme close-up, {detail}",
                "enhancers": [
                    "shallow depth of field f/2.8", "sharp focus on details",
                    "professional studio lighting", "texture details visible",
                    "hyperdetailed", "8k resolution", "photorealistic"
                ],
                "recommended_loras": ["detail_tweaker", "realistic_vision"]
            },
            "scientific_botanical": {
                "base": "scientific botanical illustration of {plant}, {detail}",
                "enhancers": [
                    "white background", "technical diagram", "educational illustration",
                    "accurate colors", "scientific accuracy", "detailed diagram"
                ],
                "recommended_loras": ["detail_tweaker", "botanical_focus"]
            },
            "natural_garden": {
                "base": "{plant} in natural garden setting, {detail}",
                "enhancers": [
                    "golden hour lighting", "soft shadows", "environmental portrait",
                    "natural sunlight", "soft focus background", "beautiful bokeh"
                ],
                "recommended_loras": ["cinematic_lighting", "realistic_vision"]
            },
            "studio_product": {
                "base": "studio photography of {plant}, {detail}",
                "enhancers": [
                    "clean white backdrop", "professional product shot",
                    "crisp details", "shadowless lighting", "high key lighting"
                ],
                "recommended_loras": ["realistic_vision"]
            }
        }
        
        self.detail_options = {
            "leaves": ["intricate vein structure", "leaf texture", "chlorophyll patterns", "stomata visible"],
            "flowers": ["pollen on stamens", "petal texture", "flower anatomy", "reproductive parts"],
            "roots": ["root system", "root hairs", "soil particles", "mycorrhizal fungi"],
            "seeds": ["seed coat texture", "germination process", "embryo visible", "seedling growth"],
            "stems": ["vascular bundles", "bark texture", "growth rings", "plant nodes"]
        }
        
        self.default_negative = (
            "cartoon, anime, painting, drawing, sketch, illustration, "
            "CGI, 3D render, computer graphics, plastic texture, artificial, "
            "blurry, out of focus, poorly drawn, bad anatomy, deformed, mutated, "
            "watermark, logo, signature, text, subtitles, UI elements, "
            "oversaturated colors, color banding, jpeg artifacts, compression, "
            "flat lighting, harsh shadows, overexposed, underexposed, "
            "unrealistic, fake, synthetic, video game graphics, "
            "withered, dead, rotten, diseased, moldy, insect damaged"
        )
    ...
    def create_botanical_prompt(self, plant_name: str, style: str = "macro_plant", 
                               plant_part: str = "leaves", custom_detail: str = None) -> Tuple[str, List[str]]:
        """Erstellt optimierte botanische Prompts mit LoRA-Empfehlungen"""
        template = self.botanical_templates.get(style, self.botanical_templates["macro_plant"])
        
        if custom_detail:
            detail = custom_detail
        else:
            detail_options = self.detail_options.get(plant_part, self.detail_options["leaves"])
            detail = random.choice(detail_options)
        
        base_prompt = template["base"].format(plant=plant_name, detail=detail)
        
        # Enhancer auswählen
        enhancers = random.sample(template["enhancers"], min(4, len(template["enhancers"])))
        
        # Kamera-Equipment
        camera_specs = [
            "shot on ARRI Alexa", "Sony A7R IV", "Canon EOS R5",
            "anamorphic lens", "35mm film", "medium format"
        ]
        camera = random.choice(camera_specs)
        
        prompt_parts = [base_prompt] + enhancers + [camera]
...
def process_botanical_book_with_loras(input_path: Path, pipeline: BotanicalSDXLLoRA, args):
    """Verarbeitet Buch mit LoRA-Optimierung"""
    ...
    # LoRAs laden (maximal 2-3 für 16GB)
    lora_names_to_load = args.loras.split(",") if args.loras else ["detail_tweaker", "realistic_vision"]
    ...
    # Header
    print("\n" + "="*80)
    print("🌿 BOTANICAL SDXL MIT LoRA-INTEGRATION (16GB OPTIMIERT)")
    print("="*80)
    print(f"📚 Buch: {title}")
    ...
    for i, scene in enumerate(scenes, 1):
        ...
        scene_style = scene.get("style", "macro_plant")
        plant_part = scene.get("plant_part", "leaves")
        custom_detail = scene.get("detail", None)
        
        # Prompt mit LoRA-Optimierung erstellen
        enhanced_prompt, recommended_loras = pipeline.create_botanical_prompt(
            base_prompt, scene_style, plant_part, custom_detail
        )
        ...
        print(f"🌱 Pflanze: {base_prompt[:60]}...")
    ...
    # Zusammenfassung
    print("\n" + "="*80)
    print("🎉 BOTANICAL LoRA-RENDERING ABGESCHLOSSEN")
    print("="*80)
...
def main():
    parser = argparse.ArgumentParser(
        description="SDXL V14 - Fotorealistische Botanische Bildgenerierung (16GB GPU optimiert)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Beispiele für 16GB GPU:
  %(prog)s --path /pfad/zum/buch --loras detail_tweaker,realistic_vision
  %(prog)s --path /pfad --width 1792 --height 1024 --steps 40
  %(prog)s --path /pfad --loras detail_tweaker --lora-scales detail_tweaker:0.7
  %(prog)s --path /pfad --start-scene 1 --end-scene 10
    ...
    parser.add_argument(
        "--loras",
        type=str,
        default="detail_tweaker,realistic_vision",
        help="Komma-getrennte LoRA-Namen (max 2-3 für 16GB)"
    )
    ...
    results, errors, output_dir = process_botanical_book_with_loras(input_path, pipeline, args)
